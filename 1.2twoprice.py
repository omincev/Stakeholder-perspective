...
def solve_offering_strategy_twoprice(capacity, lambda_DA, p_real, sys_status, alpha=0.9, beta=0):
    ...
    # Expected Profit formula (depending on one-price vs two-price scheme)
...
def plot_outcomes(result):
    ...
    ax.set_title(f"Offering outcomes (two-price)", fontsize=16)
...
result = solve_offering_strategy_twoprice(capacity, lambda_DA, p_real, sys_status)
...
print("----------------------------------------------------------------")
print(f"{'Hour':<5} | {'Expected Bid [MW]':<20} | {'Expected Profit [EUR]':<25}")
print("----------------------------------------------------------------")

expected_bid = result["Optimal Offer"]
W = len(result["Scenario Profits"])  # Number of scenarios
T = len(expected_bid)                # 24 hours

# Compute expected profit per hour
expected_profit_per_hour = np.zeros(T)
for w in range(W):
    for t in range(T):
        lambda_wt = lambda_DA[w, t]
        p_bid = expected_bid[t]
        delta = p_real[w, t] - p_bid
        sys = sys_status[w, t]

        if delta >= 0:
            if sys == 1:
                profit = lambda_wt * p_bid + 0.85 * lambda_wt * delta
            else:
                profit = lambda_wt * p_bid + lambda_wt * delta
        else:
            if sys == 1:
                profit = lambda_wt * p_bid + lambda_wt * delta
            else:
                profit = lambda_wt * p_bid + 1.25 * lambda_wt * delta
        expected_profit_per_hour[t] += profit / W

# Print each hour's result
for t in range(T):
    print(f"{t:<5} | {expected_bid[t]:<20.2f} | {expected_profit_per_hour[t]:<25.2f}")

print("\nTotal Expected Profit (Objective Function Value): {:.2f} EUR".format(result["Objective"]))


# Graph: Profit Distribution over Scenarios (Two-Price Market)
...
plt.title("Profit Distribution over Scenarios (Two-Price Market)", fontsize=14)
plt.grid(True)
plt.tight_layout()
plt.savefig("Profit_Distribution_TwoPrice.png", dpi=300)
plt.show()


# Plot cumulative distribution (CDF) of scenario profits - Two-Price Market
...
plt.title("Cumulative Distribution of Profits (CDF) two-price", fontsize=14)
plt.grid(True)
plt.tight_layout()
plt.savefig("Profit_CDF_TwoPrice.png", dpi=300)
plt.show()